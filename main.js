...
class GridGallery {
    ...
    createGrid(scroll) {
        gsap.set(this.galleryCols, {
            ...
            modifiers: {
                x: (x) => {
                    const s = gsap.utils.wrap(-this.colsBounds,
    ...
    createGridCols(element, scroll, index) {
        ...
        gsap.set(this.items, {
            ...
            modifiers: {
                y: (y) => {
                    const s = gsap.utils.wrap(-this.itemBounds,
...
class Plane {
    ...
    getDimension() {
        ...
        this.offset.set(
            left - window.innerWidth / 2 + width / 2, -top + window.innerHeight / 2 - height / 2
        );
    }
    ...
    render() {
        ...
        const strengthFactor = 0.08;
        this.uniforms.uStrength.value.set(
            Math.abs((scroll.target.x - scroll.current.x) * strengthFactor),
            Math.abs((scroll.target.y - scroll.current.y) * strengthFactor)
        );
    }
}
